...
import FormEquipment from './FormDialog.vue';
...
const searchList = computed(() => {
  return searchKey.value.length
    ? store.equipments.filter((equipment) => equipment.name.includes(searchKey.value))
    : store.equipments;
});
  ...
  <base-view title="Equipments">
    <div class="card flex flex-col space-y-6 min-h-full">
      ...
      <div class="p-4 grid grid-flow-row grid-cols-12 gap-4">
        <div class="space-y-1 text-center" v-for="(equipment, index) in searchList" :key="index">
          <div
